...
const ORDER_TYPES = [
  {
    ...
    description:
      "Napredni GPS sistem sa mogućnošću daljinskog gašenja vozila i potpunom kontrolom u realnom vremenu.",
  ...
  },
  {
    ...
    description:
      "Standardni GPS sistem za praćenje vozila u realnom vremenu sa svim osnovnim funkcijama.",
    price: "GPS sistem 5.850 rsd. jednokratno • Pretplata  rsd.",
  },
  {
    id: "renewal" as const,
...
export function OrderFormBody({
  ...
  const [paymentMethod, setPaymentMethod] = useState<"pouzece" | "racun">(
    isRenewal ? "racun" : "pouzece",
  );
  ...
  const titleId =
    variant === "modal" ? "order-modal-title" : "order-form-title";
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const addressEl = form.elements.namedItem(
      "address",
    ...
    const notes =
      notesEl instanceof HTMLTextAreaElement ? notesEl.value.trim() : "";
    const qtyRaw = (form.elements.namedItem("quantity") as HTMLInputElement)
      .value;
    const quantity = Math.min(
      99,
      Math.max(1, Number.parseInt(qtyRaw, 10) || 1),
    );
    const hasInvalidEmail =
      Boolean(email) && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    const quantityMissingRenewal =
      isRenewal && (!qtyRaw.trim() || Number.parseInt(qtyRaw, 10) < 1);
    ...
    if (missingCore || notesMissingRenewal || quantityMissingRenewal) {
      ...
      if (quantityMissingRenewal) {
        setValidationError(
          "Broj sistema za produženje je obavezno polje i mora biti najmanje 1.",
        );
        return;
      }
      setValidationError(
    ...
    if (hasInvalidEmail) {
      ...
      setInvalidTerms(false);
      setValidationError(
        "Unesite ispravnu email adresu ili ostavite polje prazno.",
      );
      return;
    }
    if (!acceptedTerms) {
  ...
  const panelBorder = validationError
  ...
  const pageGrid = pageForm
    ? {
        ...
        submit: validationError
          ...
          : "lg:col-span-2 lg:col-start-1 lg:row-start-4",
        foot: validationError
          ? "lg:col-span-2 lg:col-start-1 lg:row-start-6"
          : "lg:col-span-2 lg:col-start-1 lg:row-start-5",
      }
    : null;
      ...
      {sent ? (
        ...
        >
          ...
          <p className="mx-auto mt-3 max-w-md text-base leading-relaxed text-slate-600 dark:text-slate-300 sm:mt-4 sm:text-lg lg:max-w-lg lg:text-lg">
            Hvala vam! Uskoro vas kontaktiramo radi potvrde i brze isporuke
            sistema.
          </p>
          <button
            type="button"
        ...
        >
          {validationError && (
            ...
            >
              ...
              <p
                className={`font-medium ${pageForm ? "text-sm max-lg:text-base" : "text-xs"}`}
              >
                {validationError}
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <span className="text-lg font-semibold leading-tight sm:text-lg lg:text-base">
                      {MONTH_LABELS[m]}
                    </span>
          ...
          <div className={`${sectionCardClass} ${pageGrid?.personal ?? ""}`}>
            ...
            <div
              className={
                pageForm
                  ? "mt-4 space-y-5 max-lg:space-y-5 lg:mt-2 lg:space-y-2"
                  : "mt-4 space-y-4"
              }
            >
              <div>
          ...
          <div className={`${sectionCardClass} ${pageGrid?.payment ?? ""}`}>
            ...
            <div
              className={
                pageForm
                  ? "mt-4 space-y-5 lg:mt-2 lg:space-y-2"
                  : "mt-4 space-y-5"
              }
            >
              <div>
          ...
          <p
            className={`text-center text-sm text-slate-500 dark:text-slate-400 ${pageForm ? "max-lg:text-base lg:text-sm" : ""} ${pageGrid?.foot ?? ""}`}
          >
...
function OrderInner() {
  ...
  return (
    <>
      <section
        id="porucivanje"
        className="scroll-mt-20 bg-white px-4 pt-0 pb-8 dark:bg-black sm:px-6 sm:py-14 lg:px-8"
      >
        <div className="mx-auto max-w-6xl">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-slate-600 dark:text-slate-300">
              Izaberite GPS sistem i trajanje pretplate koje vam odgovara.
              Kompletan sistem uključuje uređaj, SIM karticu i aplikaciju, uz
              mogućnost izbora pretplate na 3, 6 ili 12 meseci, bez ugovorne
              obaveze.
            </p>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-8 lg:mt-5 lg:flex-row lg:items-stretch lg:justify-center lg:gap-5 xl:gap-7">
            ...
            <div className="order-2 mx-auto w-full max-w-lg lg:order-1 lg:mx-0 lg:flex lg:w-[26%] lg:min-w-0 lg:max-w-none lg:self-center">
              ...
              >
                ...
                <p className="mt-2 flex-1 text-sm text-slate-300 dark:text-slate-400 sm:text-sm">
                  {smartPlan.description}
                </p>
                <p className="mt-4 text-sm font-medium leading-relaxed text-[#4ade80] dark:text-[#00ff9d] sm:text-sm">
                  {smartPlan.price}
                </p>
            ...
            <div className="order-1 mx-auto w-full max-w-md lg:order-2 lg:mx-0 lg:flex lg:w-[38%] lg:min-w-0 lg:max-w-none lg:self-center">
              ...
              >
                ...
                <p className="mt-2 flex-1 text-sm text-slate-300 dark:text-slate-400 sm:text-base">
                  {proPlan.description}
                </p>
                <p className="mt-4 text-sm font-medium text-[#4ade80] dark:text-[#00ff9d] sm:text-base">
                  {proPlan.price}
                </p>
            ...
            >
              ...
              >
                ...
                <p className="mt-3 text-sm font-medium text-teal-600 dark:text-teal-400">
                  Dostupno produženje na 3, 6 ili 12 meseci.
                </p>
      ...
      {modalType && !isOrderPage && (
        <OrderModal type={modalType} onClose={() => setModalType(null)} />
      )}
    </>
  );
}
